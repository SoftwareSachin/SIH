...
export interface IStorage {
  ...
  // DSS Analytics
  getTotalRecommendations(): Promise<number>;
  getImplementedRecommendations(): Promise<number>;
  getTotalBudgetImpact(): Promise<number>;
...
export class DatabaseStorage implements IStorage {
  ...
  // DSS Analytics implementation
  async getTotalRecommendations(): Promise<number> {
    const result = await db.select({ count: count() }).from(schemeRecommendations);
    return result[0]?.count || 0;
  }

  async getImplementedRecommendations(): Promise<number> {
    const result = await db
      .select({ count: count() })
      .from(schemeRecommendations)
      .where(eq(schemeRecommendations.status, 'approved'));
    return result[0]?.count || 0;
  }

  async getTotalBudgetImpact(): Promise<number> {
    const result = await db
      .select({ total: sql<number>`sum(cast(${schemeRecommendations.estimatedBenefit} as numeric))` })
      .from(schemeRecommendations);
