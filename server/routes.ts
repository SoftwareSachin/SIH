...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/dev/add-sample-data', async (req, res) => {
    try {
      // Add sample states
      const state = await storage.createState({ name: 'Madhya Pradesh', code: 'MP' });
      const state2 = await storage.createState({ name: 'Odisha', code: 'OR' });
      
      // Add sample districts
      const district1 = await storage.createDistrict({ name: 'Dhar', stateId: state.id, code: 'DH' });
      const district2 = await storage.createDistrict({ name: 'Keonjhar', stateId: state2.id, code: 'KJ' });
      
      // Add sample villages
      const village1 = await storage.createVillage({ 
        name: 'Jamgaon', 
      ...
      const village2 = await storage.createVillage({ 
        name: 'Barbil', 
      ...
      const village3 = await storage.createVillage({ 
        name: 'Khargone', 
        districtId: district1.id, 
        pincode: '451001',
        latitude: 21.8236, 
      ...
      // Add sample scheme recommendations
      await storage.createSchemeRecommendation({
        userId: user.id,
        claimId: claim1.id,
        schemeId: 'pm-kisan',
        eligibilityScore: 85,
        matchingCriteria: ['landOwnership', 'ruralArea', 'farmerStatus'],
        recommendationReason: 'High match for PM-KISAN based on land area and farmer status',
        applicationGuidance: 'Submit Aadhaar, bank details, and land records',
      ...
      await storage.createSchemeRecommendation({
        ...
        schemeId: 'mgnrega',
        eligibilityScore: 92,
        matchingCriteria: ['ruralArea', 'employmentNeed'],
        recommendationReason: 'Excellent match for MGNREGA employment guarantee',
        applicationGuidance: 'Apply for job card at local Gram Panchayat',
      ...
      await storage.createSchemeRecommendation({
        userId: user.id,
        claimId: claim1.id,
        schemeId: 'jal-jeevan-mission',
        eligibilityScore: 78,
        matchingCriteria: ['ruralHousehold', 'waterAccess'],
        recommendationReason: 'Good match for household water connection',
        applicationGuidance: 'Village-level water committee application required',
      ...
      res.status(500).json({ message: 'Failed to add sample data', error: error.message });
    }
  });
  ...
  // Get DSS analytics and statistics
  app.get('/api/dss/analytics', async (req: any, res) => {
    try {
      // Calculate real analytics from database
      const totalRecommendations = await storage.getTotalRecommendations();
      const implementedRecommendations = await storage.getImplementedRecommendations();
      const totalBudgetImpact = await storage.getTotalBudgetImpact();
      ...
      console.error('Error fetching DSS analytics:', error);
      res.status(500).json({ message: 'Internal server error' });
    }
  });
