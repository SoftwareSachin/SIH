...
export class PythonOCRClient {
  ...
  constructor() {
    ...
    this.scriptPath = path.join(process.cwd(), 'server', 'simple_ocr.py');
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      const result = await this.runPythonScript(['-c', 'import subprocess; subprocess.run(["tesseract", "--version"], check=True); print("Simple-OCR-Ready")']);
      
      if (result.success && result.stdout.includes('Simple-OCR-Ready')) {
        console.log('✅ Simple OCR engine is ready (tesseract binary + PIL only)');
        return true;
      }
