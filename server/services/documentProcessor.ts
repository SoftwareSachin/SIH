...
`;
  ...
  private extractEntitiesBasic(text: string): any {
    ...
    // Process the cleaned text with precise patterns for structured FRA documents
    // Text format: "Name: Ram Singh Village: Garhwa State: Madhya Pradesh CLAIM DETAILS: Patta Number: 150043/4-220..."
    
    // Extract claimant names - precise pattern: "Name: Ram Singh Village"
    const nameMatch = text.match(/name:\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+village/i);
    if (nameMatch && nameMatch[1]) {
      entities.claimantNames.push(nameMatch[1].trim());
      console.log('   Found name:', nameMatch[1].trim());
    }
    
    // Extract village names - precise pattern: "Village: Garhwa State"
    const villageMatch = text.match(/village:\s*([A-Za-z\u0900-\u097F\u0980-\u09FF\u0A00-\u0A7F\u0C00-\u0C7F]+)\s+state/i);
    if (villageMatch && villageMatch[1]) {
      entities.villages.push(villageMatch[1].trim());
      entities.boundaries.push(villageMatch[1].trim());
      console.log('   Found village:', villageMatch[1].trim());
    }
    
    // Extract state names - precise pattern: "State: Madhya Pradesh CLAIM"
    const stateMatch = text.match(/state:\s*([A-Za-z\u0900-\u097F\u0980-\u09FF\u0A00-\u0A7F\u0C00-\u0C7F\s]+?)\s+claim/i);
    if (stateMatch && stateMatch[1]) {
      entities.states.push(stateMatch[1].trim());
      entities.boundaries.push(stateMatch[1].trim());
      console.log('   Found state:', stateMatch[1].trim());
    }
    
    // Extract patta/survey numbers - enhanced patterns
    const pattaMatch = text.match(/(?:patta|khata|survey|plot|ror|case)\s*(?:number|no\.?)\s*:?\s*([\d\/\-]+)/i);
